...
- (NSArray*)standardTestWithString:(NSString*)string lineWidth:(CGFloat)lineWidth font:(UIFont*)font firstLineWidth:(CGFloat)firstLineWidth limitLineCount:(int)limitLineCount {
    ...
    for (int i=0; i<testArray.count; i++) {
        ...
        if (limitLineCount==0 || i!=testArray.count-1) {
            STAssertTrue([line widthWithFont:systemFont_ maxWidth:CGFLOAT_MAX] <= limitWidth_ + safeMargin, @"Line %d is longer than limit: %f vs %f", i, [line widthWithFont:systemFont_ maxWidth:CGFLOAT_MAX], limitWidth_ + safeMargin);
        }
...
- (void)testNormalBreak {
    ...
    STAssertTrue(testArray.count == 11, @"Line break didn't give line count result: %d", testArray.count);
}
...
- (void)testInternational {
    ...
    STAssertTrue(lines.count==3, @"Japanese line break failed: %d lines", lines.count);
